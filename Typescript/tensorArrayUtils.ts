 ...
 */
export function typedArrayToArray(typedArray: tf.TypedArray | number[]): number[] {
    return Array.from(typedArray as any) as number[];
}

/**
 * Extracts data from a tensor and converts to a regular array
 */
export function tensorDataToArray(tensor: tf.Tensor | null): number[] | null {
    if (!tensor || tensor.isDisposed) return null;
    try {
        // Cast to any to avoid type issues, then cast result to number[]
        return Array.from(tensor.dataSync() as any) as number[];
    } catch (e) {
 ...
 */
export function extractScalar(tensor: tf.Tensor | null): number {
    if (!tensor || tensor.isDisposed) return 0;
    try {
 ...
 */
export function getFirstElement(tensorArray: tf.Tensor | null): number[] | number[][] | number[][][] | number[][][][] | number[][][][][] | null {
    if (!tensorArray || tensorArray.isDisposed) return null;
    try {
