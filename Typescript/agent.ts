...
import type { Tensor, Variable, Scalar, Optimizer, TensorLike, TypedArray, Rank } from '@tensorflow/tfjs-core';
import { layers as tfLayers } from '@tensorflow/tfjs-layers';
import type { Sequential, LayersModel } from '@tensorflow/tfjs-layers';

import { Config, emotionNames, HEAD_MOVEMENT_LABELS, NUM_HEAD_MOVEMENTS, HeadMovementLabel } from './config.js';
...
import { clamp, displayError, inspectTensor, norm } from './utils.js';
import { safeDispose, asNonNullTensor, containsNaNOrInf } from './tensorUtils.js';
import { asTensor, asNumberTensor } from './tensorTypeUtils.js';
import { isTensor } from './tfTypesFix.js';

// Helper function to safely cast tensors for TypeScript
function asTensor(tensor: any): Tensor {
...
interface MemoryEntry {
    timestamp: number;
    beliefEmbedding: Tensor; // Kept tensor
}
...
export interface AgentProcessResponse {
    ...
    rihScore: number;
    affinities: number[];
    emotions: Tensor; // Keep this tensor for potential downstream use, but caller should dispose
    hmLabel: HeadMovementLabel;
    responseText: string;
    trustScore: number;
    integration: number;
    reflexivity: number;
...
interface AgentState {
    ...
    error?: string;
    prevEmotions: number[];
    memoryBuffer: { timestamp: number; beliefEmbedding: number[] | null }[];
    lastRIH: number;
    ...
    selfState: number[];
    // Store weights as nested number arrays for serialization (use any[] for flexibility)
    beliefNetworkWeights: any[] | null;
    cascadeInputLayerWeights: any[] | null;
    valueHeadWeights: any[] | null;
    feedbackHeadWeights: any[] | null;
    emotionalModuleWeights: any[] | null;
    headMovementHeadWeights: any[] | null;
}
...
export class SyntrometricAgent {
    ...
    enyphansyntrix: Enyphansyntrix | null = null;
    affinitaetssyndrom: Affinitaetssyndrom | null = null;
    strukturkondensation: Strukturkondensation | null = null;
    reflexiveIntegration: ReflexiveIntegration | null = null;

    // Agent State
    memorySize: number = Config.Agent.HISTORY_SIZE || 10;
    memoryBuffer: MemoryEntry[] = []; // Holds kept tensors, managed internally
    lastRIH: number = 0.0;
    lastCascadeVariance: number = 0.0;
    latestTrustScore: number = 1.0;
    latestAffinities: number[] = [];
    latestBeliefEmbedding: Tensor | null = null; // Kept tensor, managed internally
    latestCascadeHistoryArrays: number[][] = [];
    latestRihScore: number = 0.0;

    // TensorFlow.js Components (can be null if initialization fails)
    integrationParam: Variable | null = null;
    reflexivityParam: Variable | null = null;
    selfState: Variable | null = null; // Kept variable, managed internally
    beliefNetwork: Sequential | null = null;
    // Use tf.layers.Layer type explicitly
    cascadeInputLayer: tfLayers.Layer | null = null; // tf.layers.dense returns Layer
    valueHead: tfLayers.Layer | null = null;        // tf.layers.dense returns Layer
    feedbackHead: tfLayers.Layer | null = null;     // tf.layers.dense returns Layer
    emotionalModule: Sequential | null = null;
    headMovementHead: Sequential | null = null;
    prevEmotions: Tensor | null = null; // Kept tensor, managed internally
    optimizer: Optimizer | null = null;

    private isTfReady: boolean = false;
    private coreModulesReady: boolean = false;
    ...
    private _set_tf_members_null(): void {
    ...
    private _cleanupTfMembers(): void {
        if (!this.isTfReady) return; // No TF resources to clean if TF wasn't ready

        console.log("Cleaning up Agent TF members...");

        // Helper to safely dispose tensors/variables
        const safeDispose = (item: Tensor | Variable | null | undefined): void => {
            // Check if item exists and has isDisposed property before calling dispose
            if (item && typeof (item as any).isDisposed === 'boolean' && !(item as any).isDisposed) {
                try {
                    item.dispose();
                } catch (e) {
        ...
        const safeDisposeModelOrLayer = (modelOrLayer: LayersModel | tfLayers.Layer | Sequential | null | undefined): void => {
             if (!modelOrLayer) return;
             try {
                 // Check if getWeights method exists (present in Layer and Sequential/LayersModel)
                 if (typeof (modelOrLayer as any).getWeights === 'function') {
                    // Use for...of loop for better type handling with weights
                    const weights = (modelOrLayer as any).getWeights();
                    if (Array.isArray(weights)) {
                       for (const w of weights) {
                            // Safer check for disposable tensor/variable
                           if (w && typeof (w as any).isDisposed === 'boolean' && !(w as any).isDisposed) {
                               safeDispose(w as Tensor); // Cast to Tensor/Variable for safeDispose if needed
                           }
                       }
                    }
                 }
        ...
        this.memoryBuffer.forEach((memItem: MemoryEntry) => {
    ...
    private _buildEmotionalModel(): Sequential | null {
    ...
    private _buildHeadMovementModel(): Sequential | null {
        if (!this.isTfReady) return null;
        try {
    ...
    private _updateMemory(beliefTensor: Tensor): void {
    ...
    private _computeTrust(currentBeliefEmbedding: Tensor): number {
        if (!this.isTfReady || !currentBeliefEmbedding || currentBeliefEmbedding.isDisposed) return 0.5; // Default neutral trust
        ...
        if (this.memoryBuffer.length === 0) return 1.0; // Full trust if no memory yet

        let trustValue = 0.5; // Default value
        try {
            const trustScalar = tf.tidy((): Scalar => {
                const flatCurrent = currentBeliefEmbedding; // Already flat
                const currentNorm = tf.norm(flatCurrent); // Use tf.norm
                const currentNormValue = (currentNorm as any).arraySync() as number;

                // Avoid division by zero if norm is very small
                if (currentNormValue < 1e-9) return tf.scalar(0.0); // Cannot compute similarity

                const validSimilarities: Scalar[] = [];
                for (const memEntry of this.memoryBuffer) {
                    ...
                    if (memTensor && !memTensor.isDisposed && memTensor.rank === 1 && memTensor.shape[0] === BELIEF_EMBEDDING_DIM) {
                        ...
                        const normProdValue = normProd.arraySync() as number;
                        ...
                        if (normProdValue < 1e-9) {
                            ...
                            const similarity = (tf.div(dotProduct, normProd) as any).clipByValue(-1, 1); // Use tf.div, Clip for numerical stability
                            validSimilarities.push(similarity as Scalar);
                        }
                    }
                }


                if (validSimilarities.length === 0) return tf.scalar(0.5); // Default if no valid memories
                ...
                return trust as Scalar;
            }); // End tidy

            trustValue = (trustScalar as any).arraySync() as number;
    ...
    private _learnParameters(trustScore: number, rihScore: number, cascadeVariance: number): void {
    ...
    private _updateSelfState(currentBeliefEmbedding: Tensor, trustScore: number, integrationParamValue: number): void {
    ...
    getLatestBeliefEmbedding(): Tensor | null {
    ...
    async process(
        ...
        environmentContext: { eventType: string | null; reward: number } = { eventType: null, reward: 0 }
        ...
        const defaultResponse: AgentProcessResponse = {
            ...
            emotions: this.isTfReady ? tf.zeros([1, Config.Agent.EMOTION_DIM]) : (null as any), // Handle TF not ready case
            hmLabel: 'idle',
        ...
        let cascadeHistoryArrays: number[][] = [];
        let currentRihScore = 0.0;
        let currentAffinities: number[] = [];
        let currentAvgAffinity = 0.0;
        let currentTrustScore = 0.5;
        let cascadeVarianceFeature = 0.0; // Use for learning params
        let latestBeliefEmbeddingToCache: Tensor | null = null; // Temp holder for the belief tensor to be kept

        // --- Core Syntrometric Processing in Tidy ---
        try {
            const results = tf.tidy(() => { // Primary computation block
                // --- Prepare Inputs ---
                ...
                const beliefEmbedding = this.beliefNetwork!.apply(beliefNetInput) as Tensor;
                ...
                const cascadeInput = this.cascadeInputLayer!.apply(beliefEmbedding) as Tensor;
                ...
                const historyArrays = cascadeHistoryTensors.map(t => t.arraySync() as number[]);
                ...
                let affinities: number[] = [];
                 if (cascadeHistoryTensors.length > 1) {
                     try {
                         ...
                         while(affinities.length < (cascadeHistoryTensors.length - 1)) affinities.push(0);
                     }
                 }
                ...
                const valuePred = this.valueHead!.apply(beliefEmbedding) as Tensor; // Output [1, 1]
                const feedbackSignalRaw = this.feedbackHead!.apply(beliefEmbedding) as Tensor; // Output [1, Config.DIMENSIONS]
        ...
        // --- Emotional Update ---
        let currentEmotionsTensor: Tensor; // This will be kept and returned
        try {
        ...
        let hmLabel: HeadMovementLabel = "idle";
    ...
    private async _updateEmotions(
        rawState: number[],
        environmentContext: { eventType: string | null; reward: number }
    ): Promise<Tensor> {
        ...
        let newEmotionsKept: Tensor | null = null;
        try {
            const newEmotionsResult = tf.tidy(() => { // Tidy block for prediction and blending
                ...
                const predictedEmotions = this.emotionalModule!.apply(input) as Tensor;
                ...
                ).clipByValue(0, 1); // Ensure emotions stay in [0, 1]
    ...
    private async _predictHeadMovement(
        currentEmotionsTensor: tf.Tensor | null, 
        rihScore: number, 
        avgAffinity: number
    ): Promise<{ label: HeadMovementLabel, dominantName: string }> {
        // Implementation with proper type checking
        let hmLabel: HeadMovementLabel = "idle";
        let dominantEmotionName: string = "Unknown";
        ...
        let emotionArray: number[] = [];
        try {
            // Asynchronously get the emotion values
            emotionArray = (await currentEmotionsTensor.array())[0] as number[]; // Get the first (only) batch item
        } catch (e) {
        ...
        if (dominantEmotionIndex !== -1) { // Ensure a valid dominant emotion was found
            let hmLogits: Tensor | null = null;
            try {
                hmLogits = tf.tidy(() => { // Tidy block for prediction
                    ...
                    return this.headMovementHead!.apply(input) as Tensor; // Output logits [1, NUM_HEAD_MOVEMENTS]
                }); // End tidy
                ...
                if (hmLogits && !hmLogits.isDisposed) {
                    ...
                    const hmIdx = (await hmIdxTensor.arraySync() as number[])[0];
                    hmLabel = isValidHeadMovement(HEAD_MOVEMENT_LABELS[hmIdx]) 
                        ? HEAD_MOVEMENT_LABELS[hmIdx] 
                        : "idle"; // Map index to label
    ...
    getState(): AgentState {
        ...
        const getWeightsData = (modelOrLayer: LayersModel | tfLayers.Layer | Sequential | null): any[] | null => {
             if (!modelOrLayer) return null;
             try {
                 // Check if getWeights method exists
                 if (typeof (modelOrLayer as any).getWeights !== 'function') return null;

                 const weights: Tensor[] = (modelOrLayer as any).getWeights(); // getWeights returns Tensor[], which are Variables
        ...
        const safeGetData = (tv: Tensor | Variable | null): number[] => {
            try {
                if (tv && !tv.isDisposed) {
        ...
        const safeGetScalar = (v: Variable | null): number => {
            try {
                if (v && !v.isDisposed) {
                    const data: TypedArray | number[] = v.dataSync();
                    // Check if data is array-like and has elements
                    if (data && typeof data === 'object' && data.length > 0) {
                        // Type assertion to satisfy TS, logic is sound.
                        return (data as TypedArray | number[])[0];
    ...
    private safeGetScalarParam(v: Variable | null, defaultValue: number = 0.5): number {
        try {
            if (v && !v.isDisposed) {
    ...
    loadState(state: any): boolean {
        try {
            ...
            if (state.latestTrustScore !== undefined) this.latestTrustScore = state.latestTrustScore;
            if (state.lastRIH !== undefined) this.lastRIH = state.lastRIH;
     ...
     */
    public getScalarParam(paramName: string): number {
