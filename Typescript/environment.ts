...
// Extend the TensorFlow Tensor interface to include missing methods
...
function safeDisposeTensor(tensor: tf.Tensor | null): void {
...
function safeClipByValue(tensor: tf.Tensor, min: number, max: number): tf.Tensor {
  try {
    ...
    async step(
        ...
        try {
            ...
            agentEmotionsArray = (agentEmotionsTensor && !agentEmotionsTensor.isDisposed)
                ? tensorDataToArray(agentEmotionsTensor) || zeros([this.emotionDim]) as number[]
        ...
        if (this.baseEmotions && !this.baseEmotions.isDisposed) {
            try {
                ...
                if (this.baseEmotions && !this.baseEmotions.isDisposed) safeDisposeTensor(this.baseEmotions);
    ...
    private async _updateStateVector(currentBaseEmotionsTensor: tf.Tensor | null, eventType: string | null = null): Promise<void> {
        ...
        try {
            ...
            if (baseEmotionsArray.length !== this.emotionDim) {
             ...
             try {
                 baseEmotionsArray = currentBaseEmotionsTensor && !currentBaseEmotionsTensor.isDisposed
                    ? (await currentBaseEmotionsTensor.array())[0] as number[]
                    : zeros([this.emotionDim]) as number[];
    ...
    getState(): EnvironmentState {
        ...
        try {
            ...
            baseEmotionsArray = this.baseEmotions && !this.baseEmotions.isDisposed
                ? tensorDataToArray(this.baseEmotions) || zeros([this.emotionDim]) as number[]
                : zeros([this.emotionDim]) as number[];
