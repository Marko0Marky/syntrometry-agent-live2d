 ...
 */
export function handleScriptError(library: string, fallback: string | null, message?: string): void {
 ...
 */
export function displayError(message: string, isCritical: boolean = false, elementId: string = 'error-message'): void {
 ...
 */
export function tensor(data: tf.TensorLike, shape?: number[], dtype?: tf.DataType): tf.Tensor | null {
...
export function clamp(value: number, min: number, max: number): number {
     ...
     if (!isFinite(value) || !isFinite(min) || !isFinite(max)) return NaN;
 ...
 */
export function norm(arr: number[] | tf.TypedArray | null): number { // Allow TypedArray from tensor.dataSync()
...
export function lerp(start: number, end: number, t: number): number {
     if (!isFinite(start) || !isFinite(end) || !isFinite(t)) return NaN;
 ...
 */
export function appendChatMessage(sender: string, message: string): void {
 ...
 */
export function logToTimeline(message: string, listId: string, maxItems: number = 20): void {
 ...
 */
export function inspectTensor(data: any, elementId: string): void {
...
export function clampArray(arr: number[], min: number, max: number): number[] {
     ...
     if (!isFinite(min) || !isFinite(max)) return arr;
...
export function softmax(arr: number[]): number[] {
    ...
    if (finiteArr.length === 0) return zeros([arr.length]) as number[];
    ...
    if (sumExps === 0 || !isFinite(sumExps)) {
        ...
        const result = zeros([arr.length]) as number[];
        let k = 0;
    ...
    const softmaxResult = zeros([arr.length]) as number[];
    let expIndex = 0;
    for(let i=0; i<arr.length; i++) {
        if(typeof arr[i] === 'number' && isFinite(arr[i])) {
 ...
 */
export function isTensor(value: any): value is tf.Tensor {
    return value !== null && 
           typeof value === 'object' && 
           'shape' in value && 
           'rank' in value && 
 ...
 */
export function getTensorInfo(data: any): { shape: number[], rank: number, dtype: string, size: number } | null {
    if (!isTensor(data)) {
        return null;
    }
 ...
 */
export function getCascadeLevel(cascadeHistory: any, level: number = 0): number[] {
