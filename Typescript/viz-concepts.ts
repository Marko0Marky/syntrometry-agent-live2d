// Add this at the top of the file to make it a module
export {};
...
// Remove duplicate type declarations since they're already in node_modules

// --- Define Interfaces ---
interface ConceptNodeUserData {
    id: string;
    name: string;
...
interface ConceptNodeEntry {
    object: THREE.Mesh;
    label: CSS2DObject | null;
    data: ConceptNodeUserData;
}
...
// --- Exports for app.js ---
export let conceptScene: THREE.Scene | null = null;
export let conceptCamera: THREE.PerspectiveCamera | null = null;
export let conceptRenderer: THREE.WebGLRenderer | null = null;
export let conceptLabelRenderer: CSS2DRenderer | null = null;
export let conceptControls: OrbitControls | null = null;
export let conceptInitialized: boolean = false;

// --- Internal Module Variables ---
let conceptRaycaster: THREE.Raycaster | null = null;
let conceptMouse: THREE.Vector2 | null = null;
let conceptNodes: Record<string, ConceptNodeEntry> = {};
let conceptEdges: THREE.Mesh[] = [];
let conceptContainer: HTMLElement | null = null;
let conceptInfoPanel: HTMLElement | null = null;
let conceptClock: THREE.Clock | null = null;

let agentStateMesh: THREE.Mesh | null = null;
let emergenceCoreMesh: THREE.Mesh | null = null;
let agentStateLabel: CSS2DObject | null = null;
let emergenceCoreLabel: CSS2DObject | null = null;
let live2dPlaneConcept: THREE.Mesh | null = null;

let baseConceptEdgeMaterial: THREE.MeshPhongMaterial | null = null;

let hoveredObject: THREE.Object3D | null = null;
let selectedObject: THREE.Object3D | null = null;

// Cached simulation state
let latestAgentEmotions: number[] | null = null; // Store as array
let latestRIHScore: number = 0;
let latestAvgAffinity: number = 0;
let latestTrustScore: number = 1.0;
let latestHmLabel: string = "idle"; // Use HeadMovementLabel type from config if strict

// Concept Data (assuming structure is consistent) - Type definition for conceptData entries
interface ConceptDataEntry {
    id: string;
    name: string;
    chapter: number | string; // Allow string for 'Simulation', 'Visualization'
    position: THREE.Vector3;
    type: string; // Could be improved with literal types
    links?: string[]; // Optional links array
    description: string;
}
// Type the main conceptData object
const conceptData: Record<string, ConceptDataEntry> = {
...
function getApproxBoundaryRadius(geometry: THREE.BufferGeometry | undefined, scale: number = 1.0): number {
    if (!geometry) return 1.0 * scale;
    ...
    try {
        ...
        if (!geo.boundingSphere) return 1.0 * scale;
...
export function initConceptVisualization(appClock: THREE.Clock): boolean {
    // Dependency check
    ...
    try {
        ...
        const infoToggleButton = document.getElementById('toggle-info-panel') as HTMLButtonElement | null; // Type assertion
        ...
        const interactableObjects: THREE.Object3D[] = [
            ...
            ...[agentStateMesh, emergenceCoreMesh].filter((o): o is THREE.Mesh => o !== null && o.type === 'Mesh')
        ];
...
function createConceptNodes(): void {
    ...
    for (const id in conceptData) {
        ...
        let geometry: THREE.BufferGeometry; // Use base type
        ...
        } as ConceptNodeUserData; // Type assertion
        ...
        node.add(label);
        (node.userData as ConceptNodeUserData).label = label; // Assign to typed userData

        conceptNodes[id] = { object: node, label: label, data: node.userData as ConceptNodeUserData };
    }
...
function createConceptEdges(): void {
    ...
    const allNodes: Record<string, Partial<ConceptNodeEntry>> = { ...conceptNodes }; // Use Partial as placeholders might be incomplete
    if (agentStateMesh?.userData?.id) allNodes[agentStateMesh.userData.id] = { object: agentStateMesh, data: agentStateMesh.userData as ConceptNodeUserData, label: agentStateMesh.userData.label };
    if (emergenceCoreMesh?.userData?.id) allNodes[emergenceCoreMesh.userData.id] = { object: emergenceCoreMesh, data: emergenceCoreMesh.userData as ConceptNodeUserData, label: emergenceCoreMesh.userData.label };
    if (live2dPlaneConcept?.userData?.id) allNodes[live2dPlaneConcept.userData.id] = { object: live2dPlaneConcept, data: live2dPlaneConcept.userData as ConceptNodeUserData, label: null };
    ...
    for (const id of Object.keys(allNodes)) {
        ...
        const sourceObject = sourceNodeEntry.object as THREE.Mesh; // Assume Mesh
        ...
        for (const targetId of sourceNodeEntry.data.links) {
            ...
            if (!targetNodeEntry?.object?.position) continue;

            const targetObject = targetNodeEntry.object as THREE.Mesh; // Assume Mesh
            ...
            if (curveNormal.length() * curveNormal.length() < 0.1) curveNormal = new THREE.Vector3(0, 1, 0);
            ...
            } as EdgeUserData; // Type assertion
...
function createAgentSimulationPlaceholders(): void {
    // Dispose previous meshes (similar logic as before, ensure labels are handled)
     if (agentStateMesh) {
         if (agentStateLabel && agentStateMesh.children.includes(agentStateLabel)) agentStateMesh.remove(agentStateLabel);
         ...
         if ('material' in agentStateMesh) {
             const mat = agentStateMesh.material as THREE.Material | THREE.Material[];
     ...
     if (emergenceCoreMesh) {
         if (emergenceCoreLabel && emergenceCoreMesh.children.includes(emergenceCoreLabel)) emergenceCoreMesh.remove(emergenceCoreLabel);
         ...
         if ('material' in emergenceCoreMesh) {
             const mat = emergenceCoreMesh.material as THREE.Material | THREE.Material[];
             if (Array.isArray(mat)) {
                 mat.forEach(m => m.dispose());
             } else {
                 mat.dispose();
     ...
     if (live2dPlaneConcept) {
         ...
         if ('material' in live2dPlaneConcept) {
             const mat = live2dPlaneConcept.material as THREE.Material | THREE.Material[];
             if (Array.isArray(mat)) {
                 mat.forEach(m => m.dispose());
             } else {
                 mat.dispose();
    ...
    } else { console.warn("Agent Emotional State concept data not found or scene missing."); }
    ...
    if (coreData && conceptScene) {
        ...
        emergenceCoreMesh.userData = { /* ... Fill userData as ConceptNodeUserData ... */ } as ConceptNodeUserData;
    ...
    } else { console.warn("Emergence Core concept data not found or scene missing."); }
    ...
    if (live2dData && conceptScene) {
        ...
        live2dPlaneConcept.userData = { /* ... Fill userData as ConceptNodeUserData ... */ } as ConceptNodeUserData;
         // (Copy full userData structure)
        live2dPlaneConcept.userData.originalPosition = live2dData.position.clone();
         //... etc
        conceptScene.add(live2dPlaneConcept);
    } else { console.warn("Live2D Avatar Ref concept data not found or scene missing."); }
}

/** Updates visuals of placeholder nodes based on simulation metrics. */
export function updateAgentSimulationVisuals(
    emotionsTensor: tf.Tensor | null, // Accept null
    rihScore: number,
    avgAffinity: number,
    hmLabel: string, // Consider HeadMovementLabel type
    trustScore: number
): void {
    if (!conceptInitialized || !agentStateMesh || !emergenceCoreMesh || !live2dPlaneConcept) return;

    // Store latest values for animation
    latestAgentEmotions = emotionsTensor ? emotionsTensor.arraySync() as number[] : null;
    ...
    if (agentStateMesh && latestAgentEmotions) {
        const userData = agentStateMesh.userData as ConceptNodeUserData; // Assert type
        const emotions = latestAgentEmotions;
        let dominantIndex = -1, maxIntensity = -1;
        emotions.forEach((intensity, i) => { if (intensity > maxIntensity) { maxIntensity = intensity; dominantIndex = i; } });
        const emotionAvg = emotions.reduce((sum, val) => sum + val, 0) / Config.Agent.EMOTION_DIM;

        const emotionColorMap: Record<number, number> = { 0: 0x66ff66, 1: 0xff6666, 2: 0x66ccff, 3: 0xff9966, 4: 0x99ffcc, 5: 0xffff66 };
    ...
    if (emergenceCoreMesh?.material instanceof THREE.MeshPhongMaterial && emergenceCoreMesh.userData) {
        const userData = emergenceCoreMesh.userData as ConceptNodeUserData;
    ...
    if (live2dPlaneConcept?.userData) {
        const userData = live2dPlaneConcept.userData as ConceptNodeUserData;
    ...
    conceptEdges.forEach(edge => {
        if (edge?.material instanceof THREE.MeshPhongMaterial && edge.userData) {
            const edgeUserData = edge.userData as EdgeUserData; // Assert type
...
function setupConceptInteraction(interactableObjects: THREE.Object3D[]): void {
...
function onConceptMouseMove(event: MouseEvent, interactableObjects: THREE.Object3D[]): void {
...
function onConceptClick(event: MouseEvent, interactableObjects: THREE.Object3D[]): void {
...
function onConceptKeyDown(event: KeyboardEvent): void {
    if (!conceptInitialized || !conceptNodes || Object.keys(conceptNodes).length === 0 || !conceptControls) return;

    const allNodeIds = Object.keys(conceptNodes);
    const placeHolderIds = [agentStateMesh?.userData?.id, emergenceCoreMesh?.userData?.id].filter((id): id is string => !!id);
...
export function updateInfoPanel(): void {
    ...
    const infoToggleButton = document.getElementById('toggle-info-panel') as HTMLButtonElement | null;
    if (infoToggleButton) infoToggleButton.setAttribute('aria-expanded', String(shouldBeVisible));

    if (displayObject && displayObject.userData?.id) {
        const data = displayObject.userData as ConceptNodeUserData; // Assert type
        ...
        const emotions = latestAgentEmotions || zeros([Config.Agent.EMOTION_DIM]) as number[];
        let dominantIndex = -1, maxIntensity = -1;
        emotions.forEach((intensity, i) => { if (intensity > maxIntensity) { maxIntensity = intensity; dominantIndex = i; } });
        ...
        if (data.links && data.links.length > 0) {
            ...
            data.links.forEach(linkId => {
                // Find link data (combine placeholders and concepts)
                 const allNodesForLinkLookup: Record<string, Partial<ConceptNodeEntry>> = { ...conceptNodes };
                 if (agentStateMesh?.userData?.id) allNodesForLinkLookup[agentStateMesh.userData.id] = { data: agentStateMesh.userData as ConceptNodeUserData };
                 if (emergenceCoreMesh?.userData?.id) allNodesForLinkLookup[emergenceCoreMesh.userData.id] = { data: emergenceCoreMesh.userData as ConceptNodeUserData };
                 if (live2dPlaneConcept?.userData?.id) allNodesForLinkLookup[live2dPlaneConcept.userData.id] = { data: live2dPlaneConcept.userData as ConceptNodeUserData };
...
export function animateConceptNodes(
    ...
    const allNodesToAnimate: THREE.Object3D[] = [ // Use base Object3D type
        ...Object.values(conceptNodes).map(n => n.object),
        agentStateMesh,
        emergenceCoreMesh,
        live2dPlaneConcept,
    ].filter((o): o is THREE.Mesh => o instanceof THREE.Mesh); // Filter and assert Mesh

    // --- Animate Each Node ---
    allNodesToAnimate.forEach(object => {
        // Ensure material is MeshPhongMaterial for color/emissive access
        if (!object || !(object instanceof THREE.Mesh) || !object.userData?.originalPosition) return;

        const data = object.userData as ConceptNodeUserData; // Assert type
        ...
        let feedbackEmissiveColor: THREE.Color | null = null;
        ...
        if (isSelected) {
            ...
            targetScale.multiplyScalar(1.2); targetOpacity = Math.min(baseOpacity + 0.3, 1.0);
        } else if (isHovered) {
            targetColor.lerp(nodeHighlightColor, 0.4); targetEmissive.lerp(nodeHighlightEmissive, 0.5);
            targetScale.multiplyScalar(1.1); targetOpacity = Math.min(baseOpacity + 0.2, 1.0);
        } else if (isLinkedToSelected) {
            targetColor.lerp(linkedColor, 0.5); targetEmissive.lerp(linkedEmissive, 0.6);
            targetScale.multiplyScalar(1.05); targetOpacity = Math.min(baseOpacity + 0.15, 1.0);
        } else if (isLinkedToHovered) {
            targetColor.lerp(linkedColor, 0.3); targetEmissive.lerp(linkedEmissive, 0.4);
            targetScale.multiplyScalar(1.025); targetOpacity = Math.min(baseOpacity + 0.1, 1.0);
        }
        ...
        if (object instanceof THREE.Mesh) {
            ...
            if (material instanceof THREE.Material) {
                if ('color' in material && material.color) {
                    // Ensure color is a Color before calling lerp
                ...
                if ('emissive' in material && material.emissive) {
                    // Ensure emissive is a Color before calling lerp
    ...
    // --- Animate Edges ---
    conceptEdges.forEach(edge => {
        if (!edge?.material || !(edge.material instanceof THREE.MeshPhongMaterial) || !edge.userData) return;
        ...
        if (isSelectedEdge) {
            targetEdgeColor.lerp(edgeHighlightColor, 0.7); targetEdgeEmissive.lerp(edgeHighlightEmissive, 0.8);
            targetEdgeOpacity = Math.min(baseOpacity + 0.4, 0.95);
        } else if (isHoveredEdge) {
            targetEdgeColor.lerp(edgeHighlightColor, 0.5); targetEdgeEmissive.lerp(edgeHighlightEmissive, 0.6);
        ...
        if (edge?.material) {
            const material = edge.material as THREE.MeshPhongMaterial;
            if (material.color && material.emissive) {
                material.color.lerp(targetEdgeColor, lerpFactor);
                material.emissive.lerp(targetEdgeEmissive, lerpFactor);
                material.opacity = lerp(material.opacity, targetEdgeOpacity, lerpFactor);
                material.transparent = material.opacity < 1.0;
            }
        }
    });
...
// Cleans up resources
export function cleanupConceptVisualization(): void {
    if (!conceptScene) return; // Already cleaned
    ...
    try {
        ...
        if (infoToggleButton) (infoToggleButton as HTMLButtonElement).onclick = null;
        ...
        conceptScene = null; conceptCamera = null; conceptRenderer = null;
        conceptLabelRenderer = null; conceptControls = null; conceptRaycaster = null;
        conceptMouse = null; conceptNodes = {}; conceptEdges = []; conceptContainer = null;
        conceptInfoPanel = null; conceptClock = null; agentStateMesh = null;
        emergenceCoreMesh = null; agentStateLabel = null; emergenceCoreLabel = null;
        live2dPlaneConcept = null; baseConceptEdgeMaterial = null; hoveredObject = null;
        selectedObject = null; latestAgentEmotions = null;
        conceptInitialized = false;
    }
}

// Handles window resize events
function onConceptWindowResize(): void {
...
function isMesh(obj: THREE.Object3D | null): obj is THREE.Mesh {
    return obj !== null && obj.type === 'Mesh';
}

// Wrapper functions for event listeners
function handleConceptMouseMoveWrapper(event: MouseEvent): void {
    const conceptInteractableObjects: THREE.Object3D[] = [
        ...Object.values(conceptNodes).map(n => n.object),
        ...[agentStateMesh, emergenceCoreMesh, live2dPlaneConcept].filter((o): o is THREE.Mesh => o !== null && o.type === 'Mesh')
    ];
    onConceptMouseMove(event, conceptInteractableObjects);
}

function handleConceptClickWrapper(event: MouseEvent): void {
    const conceptInteractableObjects: THREE.Object3D[] = [
        ...Object.values(conceptNodes).map(n => n.object),
        ...[agentStateMesh, emergenceCoreMesh, live2dPlaneConcept].filter((o): o is THREE.Mesh => o !== null && o.type === 'Mesh')
    ];
...
function handleConceptKeyDownWrapper(event: KeyboardEvent): void {
 ...
 */
function getMeshMaterial(object: THREE.Object3D): THREE.Material | null {
