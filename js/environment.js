        ...
        if (this.baseEmotions && !this.baseEmotions.isDisposed && typeof tf !== 'undefined') {
            try {
                const updatedBaseEmotions = tf.tidy(() => {
                     ...
                     const driftToAgent = agentEmotions.sub(currentBase).mul(Config.Env.BASE_EMOTION_DRIFT_RATE);
                     // Revert towards neutral (0.5): base + (0.5 - base) * revert_rate
                     const revertToNeutral = tf.scalar(0.5).sub(currentBase).mul(Config.Env.BASE_EMOTION_REVERSION_RATE);
             ...
             // If TF exists but tensor is null/disposed, re-initialize
