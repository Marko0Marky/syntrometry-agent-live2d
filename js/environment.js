...
export class EmotionalSpace {
     ...
     */
        // Inside the EmotionalSpace class...
        ...
        async step(agentEmotionsTensor, currentRIHScore = 0, currentAvgAffinity = 0) {
            ...
            const driftRate = Config?.Env?.BASE_EMOTION_DRIFT_RATE ?? 0; // Get value, default to 0 if undefined
            const reversionRate = Config?.Env?.BASE_EMOTION_REVERSION_RATE ?? 0; // Get value, default to 0 if undefined
    
            // Optional: Log here to confirm values if debugging needed
            // console.log('Check Config in Env Step:', { driftRate, reversionRate });
            // --- END FIX ---
            ...
            if (this.baseEmotions && !this.baseEmotions.isDisposed && typeof tf !== 'undefined') {
                try {
                    const updatedBaseEmotions = tf.tidy(() => {
                         ...
                         // --- FIX: Use the local constants inside tidy ---
                         // Drift towards agent: base + (agent - base) * drift_rate
                         const driftToAgent = agentEmotions.sub(currentBase).mul(driftRate); // Use driftRate constant
                         // Revert towards neutral (0.5): base + (0.5 - base) * revert_rate
                         const revertToNeutral = tf.scalar(0.5).sub(currentBase).mul(reversionRate); // Use reversionRate constant
                         // --- END FIX ---
            ...
            // --- Return Step Results ---
            const stateTensor = this._getStateTensor();
            const done = false;
