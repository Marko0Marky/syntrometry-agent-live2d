...
// Explicitly import the Tensor type for better type resolution
import { Tensor } from '@tensorflow/tfjs';
import { Config, emotionKeywords, emotionNames } from './config.js'; // Typed config
import { zeros, tensor, clamp, displayError } from './utils.js'; // Typed utils
import {
  fixLine170,
  fixLine191,
  fixLine192,
  ...
  fixLine459,
  fixLine545
} from './environmentFixes.js';
import { tensorDataToArray } from './tensorArrayUtils.js';

// Interface for the state saved/loaded by getState/loadState
interface EnvironmentState {
    ...
    eventTimer: number;
    gapTimer: number;
    currentEvent: { type: string; context: string; reward: number } | null;
}
...
export class EmotionalSpace {
    private events: [string, string, number][];
    private emotionNames: string[];
    baseEmotions: tf.Tensor | null = null; // Shape [1, EMOTION_DIM]
    stepCount: number = 0;
    eventTimer: number = 0;
    gapTimer: number = 0;
    currentEvent: { type: string; context: string; reward: number } | null = null;
    currentStateVector: number[] = []; // Full state vector [BASE_STATE_DIM]

    // Store validated config values internally, initialized with defaults
    private eventFreq: number = getConfigValue('Env.EVENT_FREQ', 0.015);
    private eventDuration: number = getConfigValue('Env.EVENT_DURATION', 120);
    private eventGap: number = getConfigValue('Env.EVENT_GAP', 180);
    private driftRate: number = getConfigValue('Env.BASE_EMOTION_DRIFT_RATE', 0.005);
    private reversionRate: number = getConfigValue('Env.BASE_EMOTION_REVERSION_RATE', 0.001);
    private dimensions: number = getConfigValue('DIMENSIONS', 12);
    private emotionDim: number = getConfigValue('Agent.EMOTION_DIM', 6);
    private baseStateDim: number = this.dimensions + this.emotionDim; // Initial calculation based on defaults
    private dysvariantProb: number = getConfigValue('DYSVARIANT_PROB', 0.02);
    ...
    private _validateAndStoreConfig(): void {
    ...
    private _initializeState(): void {
        ...
        this.currentStateVector = zeros([this.baseStateDim]) as number[];
    ...
    async reset(): Promise<{ state: tf.Tensor | null }> {
    ...
    async step(
        agentEmotionsTensor: tf.Tensor | null,
        currentRIHScore: number = 0,
        currentAvgAffinity: number = 0
    ): Promise<{ state: tf.Tensor | null; reward: number; done: boolean; context: string; eventType: string | null }> {
        this.stepCount++;
        let reward: number = 0;
        let context: string = "Ambient fluctuations.";
        let triggeredEventType: string | null = null;
        let agentEmotionsArray: number[];
        ...
        try {
            ...
            agentEmotionsArray = (agentEmotionsTensor && !agentEmotionsTensor.isDisposed)
                ? tensorDataToArray(agentEmotionsTensor)
                : zeros([this.emotionDim]) as number[];
            ...
            if (agentEmotionsArray.length !== this.emotionDim) {
                 ...
                 agentEmotionsArray = zeros([this.emotionDim]) as number[];
            }
        } catch (e) {
            console.error("Error reading agent emotions tensor in environment step:", e);
            agentEmotionsArray = zeros([this.emotionDim]) as number[]; // Fallback
        }
     ...
     */
    private async _updateStateVector(currentBaseEmotionsTensor: tf.Tensor | null, eventType: string | null = null): Promise<void> {
        let baseEmotionsArray: number[];
        try {
            // Prefer dataSync for synchronous access if tensor is ready, fallback to async data()
             baseEmotionsArray = currentBaseEmotionsTensor && !currentBaseEmotionsTensor.isDisposed
                ? tensorDataToArray(currentBaseEmotionsTensor)
                : zeros([this.emotionDim]) as number[];
            ...
            if (baseEmotionsArray.length !== this.emotionDim) {
                 ...
                 baseEmotionsArray = zeros([this.emotionDim]) as number[];
            }
        } catch(e) {
             // If dataSync fails (e.g., tensor not on CPU), try async
             try {
                 baseEmotionsArray = currentBaseEmotionsTensor && !currentBaseEmotionsTensor.isDisposed
                    ? await currentBaseEmotionsTensor.data() as number[] // Fallback to data()
                    : zeros([this.emotionDim]) as number[];
                ...
                if (baseEmotionsArray.length !== this.emotionDim) {
                 ...
                 baseEmotionsArray = zeros([this.emotionDim]) as number[]; // Final fallback
             }
        }
        ...
        while (this.currentStateVector.length < this.baseStateDim) this.currentStateVector.push(0.0);
    ...
    private async _getStateTensor(): Promise<tf.Tensor | null> {
        ...
        try {
            ...
            const hasInvalidValues = await tf.any(tf.logicalOr(tf.isNaN(stateTensor), tf.isInf(stateTensor))).data();
            if (hasInvalidValues[0]) {
                console.error("State tensor contains NaN or Inf values!", finalStateArray);
                tf.dispose(stateTensor); // Dispose invalid tensor
    ...
    async getEmotionalImpactFromText(text: string): Promise<tf.Tensor | null> {
        if (!tf?.tensor) {
            console.error("TensorFlow not available for text impact calculation.");
            return null;
        }

        const impact = zeros([this.emotionDim]) as number[];
        let foundKeyword = false;
        const lowerText = text.toLowerCase();
        let baseEmotionsChanged = false;
        let currentBaseArray: number[]; // Will hold numeric array

        // Get current base emotions safely
        try {
             if (this.baseEmotions && !this.baseEmotions.isDisposed) {
                 currentBaseArray = this.baseEmotions.dataSync() as number[]; // Try sync first
                 if (currentBaseArray.length !== this.emotionDim) {
                     ...
                     currentBaseArray = zeros([this.emotionDim]) as number[];
                 }
             } else {
             ...
             try {
                  ...
                  currentBaseArray = this.baseEmotions && !this.baseEmotions.isDisposed
                    ? await this.baseEmotions.data() as number[]
                    : zeros([this.emotionDim]) as number[];
                 if (currentBaseArray.length !== this.emotionDim) {
                 ...
                 currentBaseArray = zeros([this.emotionDim]) as number[]; // Final fallback
             }
        }
        ...
        for (const idxStr in emotionKeywords) {
            ...
            if (!isNaN(idx) && idx >= 0 && idx < this.emotionDim) {
                ...
                if (!info || !Array.isArray(info.keywords)) continue; // Skip if data is malformed
        ...
        if (!foundKeyword) {
            ...
            if (2 < this.emotionDim) impact[2] = Math.max(impact[2] ?? 0, 0.3);
            if (4 < this.emotionDim) impact[4] = Math.max(impact[4] ?? 0, 0.2);
        }
    ...
    getState(): EnvironmentState {
        ...
        try {
            ...
            baseEmotionsArray = this.baseEmotions && !this.baseEmotions.isDisposed
                ? tensorDataToArray(this.baseEmotions)
                : zeros([this.emotionDim]) as number[];
    ...
    loadState(state: any): void { // Use 'any' for loading flexibility, then validate
        ...
        if (Array.isArray(state.currentStateVector) && state.currentStateVector.every((v: unknown) => typeof v === 'number')) {
        ...
        let loadedBaseEmotionsArray: number[];
        if (Array.isArray(state.baseEmotions) && state.baseEmotions.every((v: unknown) => typeof v === 'number')) {
             if (state.baseEmotions.length === this.emotionDim) {
                 ...
                 loadedBaseEmotionsArray = zeros([this.emotionDim]) as number[];
             }
        } else {
