// js/scripts/fix-tf-wrapper.js
// Script to add a TensorFlow.js wrapper to app.js
...
`;
  ...
  const insertPos = content.indexOf('function isTensorFlowReady()');
  if (insertPos === -1) {
    console.error("Could not find isTensorFlowReady function");
    return false;
  }

  // Replace the isTensorFlowReady function with our wrapper
  const functionRegex = /function isTensorFlowReady\(\)[^}]*}/;
  const newContent = content.replace(functionRegex, tfWrapper);
