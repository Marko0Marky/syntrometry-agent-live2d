...
import type { Tensor } from '@tensorflow/tfjs-core'; // Use specific TF type
...
let metricsChart: Chart<'line', ChartPoint[], number> | null = null;
...
async function initialize(): Promise<void> {
    ...
    if (live2dInitialized) {
        try {
            if (initialFrameEmotions) updateLive2DEmotions(initialFrameEmotions);
...
function initMetricsChart(): void {
    ...
    const chartConfig: ChartConfiguration<'line', ChartPoint[], number> = {
        type: 'line',
        data: {
            datasets: [
        ...
        options: {
            ...
            plugins: {
                legend: {
                    ...
                    onClick: function(e: ChartEvent, legendItem: LegendItem, legend: LegendElement<'line'>) {
                ...
                tooltip: { // Use SafeTooltipItem type
                    ...
                    callbacks: {
                        title: (tooltipItems: TooltipItem<'line'>[]) => { // Keep specific TooltipItem here
...
async function loadState(showMessages: boolean = false): Promise<boolean> {
    ...
    try {
        ...
        updateCascadeViewer();
        if (agent.selfState) updateHeatmap(agent.selfState.dataSync(), 'heatmap-content');
        ...
        if (live2dInitialized) { // Live2D
            if (loadFrameEmotions) updateLive2DEmotions(loadFrameEmotions);
            updateLive2DHeadMovement(simulationMetrics.currentHmLabel, 0);
