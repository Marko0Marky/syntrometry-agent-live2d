...
// Assuming tf is imported/available globally for types
...
// Add missing THREE.js type declarations
...
// Extend OrbitControls with dispose method
...
// --- Define Interfaces ---
// (Ideally move to a central types.ts file)
interface ConceptNodeUserData {
    id: string;
    name: string;
    chapter: number | string;
    position: THREE.Vector3;
    type: string; // Consider literal type: 'framework' | 'structure' | ...
...
interface ConceptNodeEntry {
    object: THREE.Mesh; // Or potentially THREE.Object3D if placeholders are different
    label: CSS2DObject | null;
...
let conceptNodes: Record<string, ConceptNodeEntry> = {}; // Use Record for typed map
let conceptEdges: THREE.Mesh[] = []; // Edges are Meshes (TubeGeometry)
...
let hoveredObject: THREE.Object3D | null = null; // Can be Mesh or other Object3D
...
export function initConceptVisualization(appClock: THREE.Clock): boolean {
    ...
    try {
        ...
        const interactableObjects: THREE.Object3D[] = [
            ...
            agentStateMesh,
            emergenceCoreMesh,
        ].filter((o): o is THREE.Mesh => o !== null); // Use type guard and filter nulls
...
function createAgentSimulationPlaceholders(): void {
    ...
    if (agentData && conceptScene) { // Check scene existence
        ...
        agentStateMesh.userData = { /* ... Fill userData as ConceptNodeUserData ... */ } as ConceptNodeUserData; // Fill and assert
...
function handleConceptKeyDownWrapper(event: KeyboardEvent): void { // Type event
     ...
     const placeHolderIds = [agentStateMesh?.userData?.id, emergenceCoreMesh?.userData?.id].filter((id): id is string => !!id); // Filter and type guard
...
let handleConceptMouseMoveWrapper = (event: MouseEvent): void => { // Type event
    let conceptInteractableObjects: THREE.Object3D[] = [
        ...Object.values(conceptNodes).map(n => n.object),
        agentStateMesh,
        emergenceCoreMesh,
    ].filter((o): o is THREE.Mesh => o !== null); // Filter nulls and assert Mesh
    onConceptMouseMove(event, conceptInteractableObjects);
};

let handleConceptClickWrapper = (event: MouseEvent): void => { // Type event
    let conceptInteractableObjects: THREE.Object3D[] = [
        ...Object.values(conceptNodes).map(n => n.object),
        agentStateMesh,
        emergenceCoreMesh,
...
function getMeshMaterial(obj: THREE.Object3D | null): THREE.Material | null {
    if (!obj) return null;
