...
// Define interfaces for nested structures for better type safety
interface AgentConfig {
    BASE_STATE_DIM: number; // Will be calculated below
    EMOTION_DIM: number;
    HIDDEN_DIM: number;
    HISTORY_SIZE: number;
}
...
interface EnvConfig {
    ...
    EVENT_GAP: number;
    BASE_EMOTION_DRIFT_RATE: number;
    BASE_EMOTION_REVERSION_RATE: number;
}
...
interface NodeVisualizationConfig {
    BaseSize: number;
    TypeSettings: {
        [key: string]: NodeTypeSettings; // Index signature for various node types
    };
}

interface EdgeVisualizationConfig {
    TubeRadius: number;
...
interface VisualizationConfig {
    Node: NodeVisualizationConfig;
    Edge: EdgeVisualizationConfig;
}
...
interface AppConfig {
    ...
    RIH_SCALE: number;
    TELE_THRESHOLD: number;
    DYSVARIANT_PROB: number;
    Agent: AgentConfig;
    Env: EnvConfig;
    RL: RLConfig;
...
export const Config: AppConfig = {
    ...
    Agent: {
        ...
        BASE_STATE_DIM: 0, // Placeholder, calculated later
        EMOTION_DIM: 6,
        HIDDEN_DIM: 64,
        HISTORY_SIZE: 15,
    },
...
/**
 * Defines emotion keywords and their properties.
 */
interface EmotionKeywordInfo {
    name: string;
    keywords: string[];
    strength: number;
    baseChange: number;
}
export const emotionKeywords: Record<number, EmotionKeywordInfo> = {
 ...
 */
export const emotionNames: string[] = Object.values(emotionKeywords).map(e => e.name);
 ...
 */
export const HEAD_MOVEMENT_LABELS = ["nod", "shake", "tilt_left", "tilt_right", "idle"] as const;

// Define HeadMovementLabel type from the literal values
export type HeadMovementLabel = typeof HEAD_MOVEMENT_LABELS[number];

export const NUM_HEAD_MOVEMENTS: number = HEAD_MOVEMENT_LABELS.length;
