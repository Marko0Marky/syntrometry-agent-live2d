...
import { Config, emotionNames, HEAD_MOVEMENT_LABELS } from './config.js';
...
import { safeDispose, tensorToArray, tensorToArrayAsync } from './tensorUtils.js'; // Use consolidated utils
...
import { initThreeJS, updateThreeJS, cleanupThreeJS, updateSyntrometryInfoPanel, threeInitialized, calculateGraphFeatures } from './viz-syntrometry.js';
import { initConceptVisualization, updateAgentSimulationVisuals, animateConceptNodes, cleanupConceptVisualization, conceptInitialized, conceptRenderer, conceptLabelRenderer, conceptScene, conceptCamera, conceptControls } from './viz-concepts.js';
...
function isValidHeadMovement(label) {
    return HEAD_MOVEMENT_LABELS.includes(label);
}
...
function getElement(id) {
    ...
    if (el && el instanceof HTMLElement) { // Basic check
        return el; // Use cast carefully, or check specific type like HTMLInputElement
    }
    return null;
}
...
function updateMetricsChart() {
    if (!metricsChart || criticalError || agent === null)
        return;
    const now = Date.now();
    try {
        // Ensure datasets exist before pushing
        if (metricsChart.data.datasets.length >= 5) {
...
function updateDashboardDisplay() {
    const updateElement = (id, value, text = null, isProgress = false, range = [0, 1]) => {
...
async function updateEmotionBars(emotionsTensor) {
...
async function initialize() {
    ...
    if (coreInitSuccess && !initialStateLoaded && agent && environment) {
        ...
        try {
            const envResetResult = await environment.reset();
            ...
            const initialAgentResponse = await agent.process(simulationMetrics.currentStateVector, initialGraphFeatures, { eventType: null, reward: 0 });
    ...
    if (threeInitialized) {
        try {
            ...
            console.error("Error during initial syntrometry viz update:", e);
        }
    }
    ...
    if (initialFrameEmotions)
        await updateEmotionBars(initialFrameEmotions); // await UI update
    updateCascadeViewer();
    ...
    initializeDraggablePanels('.overlay-panel', '.visualization-container', ['input', 'button', 'textarea', 'select', 'progress', 'canvas', '.no-drag', '[role="button"]', 'a', 'pre', '.chart-container', '.cv-syndrome-container', '.label'], // Add .label to prevent drag
    ...
    const chartConfig = {
        ...
        options: {
            ...
            scales: {
                ...
                yPercentage: { beginAtZero: true, max: 1.0, position: 'left', ticks: { color: textColor, font: { size: 10 }, stepSize: 0.25, callback: value => `${(Number(value) * 100).toFixed(0)}%` }, grid: { color: chartGridColor } },
...
function setupControls() {
    ...
    if (labelToggle) {
        const toggleLabels = (show) => {
            if (conceptScene) { // Check if concept viz is initialized
    ...
    try {
        ...
        const stateToSave = {
            ...
            metrics: {
                ...
                intParam: simulationMetrics.currentIntegrationParam,
...
async function loadState(showMessages = false) {
    ...
    try {
        ...
        simulationMetrics.currentSelfStateNorm = agent.selfState ? norm(agent.selfState.dataSync()) : 0.0; // Calc from loaded state
        // --- Reset / Update UI ---
        if (metricsChart) {
...
async function animate() {
    ...
    if (agent && environment && simulationMetrics.currentAgentEmotions && !simulationMetrics.currentAgentEmotions.isDisposed) {
        ...
        try {
            ...
            if (simulationMetrics.currentAgentEmotions && !simulationMetrics.currentAgentEmotions.isDisposed) {
                ...
                currentFrameEmotions = tf.keep(simulationMetrics.currentAgentEmotions.clone()); // Clone the zeros
    ...
    try {
        if (conceptInitialized) {
            ...
            animateConceptNodes(deltaTime, simulationMetrics.currentIntegrationParam, simulationMetrics.currentReflexivityParam, elapsedTime - lastIntegrationInputTime, // Pass raw time difference
    ...
    try {
        if (live2dInitialized && currentFrameEmotions) {
            updateLive2DEmotions(currentFrameEmotions); // Pass frame clone
