...
export function initThreeJS() {
    if (typeof THREE === 'undefined' || typeof THREE.CSS2DRenderer === 'undefined' || typeof THREE.CSS2DObject === 'undefined') {
        displayError("Three.js or CSS2DRenderer/CSS2DObject not loaded for Syntrometry panel.", false, 'error-message');
    ...
    try {
        ...
        const height = syntrometryContainer.clientHeight;
         if (width <= 0 || height <= 0) {
             // displayError("Syntrometry panel has zero dimensions. Visualization cannot be rendered.", false, 'syntrometry-error-message'); // Reduce noise
...
function onSyntrometryMouseMove(event, interactableObjects) {
    ...
    const intersects = syntrometryRaycaster.intersectObjects(interactableObjects, false); // `false` for non-recursive check
...
export function updateSyntrometryInfoPanel() {
    ...
    if (displayObject?.userData) {
        ...
        if (data.type === 'rih_node') {
            ...
            const value = (latestStateVector && latestStateVector.length > dimIndex && typeof latestStateVector[dimIndex] === 'number')
                 ? latestStateVector[dimIndex].toFixed(3)
                 : 'N/A';
            // Example: Could update a tooltip with `Dim ${dimIndex}: ${value}`
            // console.log(`Info Panel Focus: Dimension ${dimIndex} (Value: ${value})`); // Reduce noise
        }
    } else {
...
export function updateThreeJS(deltaTime, stateVector, rihScore, affinities, integrationParam, reflexivityParam, cascadeHistory, context) {
    ...
    for (let i = 0; i < Config.DIMENSIONS; i++) {
        const node = nodes[i];
        // Added check for material being MeshPhongMaterial for emissive/color properties
        if (!node?.material?.color || !(node.material instanceof THREE.MeshPhongMaterial) || !node.userData?.originalPosition) continue;
        ...
        if (node.userData.label instanceof THREE.CSS2DObject) { // Check type
             const label = node.userData.label;
             label.position.y = (nodeBaseScale * 1.5) * node.scale.y;
             label.element.style.opacity = String(clamp(node.material.opacity ?? 1.0, 0.2, 1.0)); // Ensure string type
             label.element.style.display = parseFloat(label.element.style.opacity) < 0.25 ? 'none' : 'block';
        }
   } // End of dimension node loop

    // --- Animate RIH Node ---
    if (rihNode?.material?.color && (rihNode.material instanceof THREE.MeshPhongMaterial) && rihNode.userData?.originalPosition) {
         ...
         if (isSelected) { targetColor.copy(nodeHighlightColor); targetEmissive.copy(nodeHighlightEmissive); targetScale.set(1.4, 1.4, 1.4); targetOpacity = 1.0; }
         else if (isHovered) { targetColor.copy(nodeHighlightColor); targetEmissive.copy(nodeHighlightEmissive); targetScale.set(1.2, 1.2, 1.2); targetOpacity = 1.0; }
...
function createEdge(nodeA, nodeB, metric, param, highlightColor) {
     if (!baseEdgeMaterial || !edgesGroup || !nodeA?.material || !nodeB?.material) return; // Guard against nulls
     ...
     const edgeMaterial = baseEdgeMaterial.clone(); // Clone for individual styling
     ...
     if (isEdgeSelected || isEdgeHovered) {
     ...
     } else if (nodeA.material.color && nodeB.material.color) {
         ...
         targetEdgeColor.lerpColors(startColor, endColor, blendFactor); // Use THREE's lerpColors
     }
     ...
     const edgeLine = new THREE.Line(geometry, edgeMaterial); // Use THREE.Line
...
export function cleanupThreeJS() {
    ...
    nodes = []; edgesGroup = null; rihNode = null; scene = null; camera = null; renderer = null;
    labelRenderer = null; syntrometryContainer = null; hoveredDimension = null; selectedDimension = null;
    latestStateVector = null; baseEdgeMaterial = null;
    threeInitialized = false;
