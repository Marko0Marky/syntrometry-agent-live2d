...
export function animateConceptNodes(
    ...
    if (!conceptInitialized || !conceptClock) return;

    const elapsedTime = conceptClock.getElapsedTime();
    const inputFeedbackDuration = 0.5;

    // Create default emotions if latestAgentEmotions is null or undefined
    let emotions = [0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5]; // Default neutral emotions
    
    // Only try to use latestAgentEmotions if it exists and has data
    if (latestAgentEmotions && Array.isArray(latestAgentEmotions) && latestAgentEmotions.length > 0) {
        emotions = latestAgentEmotions;
    }
    
    // Calculate average emotion for visualization
    const emotionAvg = emotions.reduce((a, b) => a + b, 0) / (emotions.length || 1);
